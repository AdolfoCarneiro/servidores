...
function brassMachine(event) {
	...
	event.remove({output: CI('deposit_scanner')})
	event.shaped(CI('deposit_scanner'), [
		' HS',
		'HTH',
		'TMR'
	], {M: KJ('brass_machine'), H: 'alloyed:steel_sheet', R: CR('copper_sheet'), S: Item.of('createindustry:screwdriver').ignoreNBT(), T: KJ('diamond_tube')})  .damageIngredient(CI('screwdriver'),10)

	event.remove({output:  CI('cast_iron_distillation_controller')})
	event.shaped( CI('cast_iron_distillation_controller'), [
		'CPC',
		'CMC',
		'CCC'
	], {M: KJ('brass_machine'), C: BC('cast_iron_ingot'), P: CI('industrial_pipe')})

	event.smithing(CI('distillation_tower_controller'), CI('cast_iron_distillation_controller'), CI('heavy_machinery_casing'))
